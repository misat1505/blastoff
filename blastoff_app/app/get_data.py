...
async def get_api_data(db):
    ...
    try:
        ...
        for flight_data in data:
            ...
            flight_data.launch['site_id'] = flight_data.site.get('id') if flight_data.site is not None else None
            flight_data.launch['program_id'] = flight_data.program.get('id') if flight_data.program is not None else None
            ...
            if flight_data.site is not None:
                ...
                if await update_site(db=db, site_id=flight_data.site.get('id'), site_data=site_data) is None:
            ...
            if flight_data.program is not None:
                ...
                if await update_program(db=db, program_id=flight_data.program.get('id'), program_data=program_data) is None:
                    await create_program(db=db, program_data=program_data)
                    logging.info(f'Added program - {flight_data.program.get('id')}')

            agency_data = AgencyCreate(**flight_data.agency)
            if await update_agency(db=db, agency_id=flight_data.agency.get('id'), agency_data=agency_data) is None:
            ...
            if await update_rocket(db=db, rocket_id=flight_data.rocket.get('id'), rocket_data=rocket_data) is None:
            ...
            if await update_launch(db=db, launch_id=flight_data.launch.get('id'), launch_data=launch_data) is None:
