...
@given(st.text(max_size=0), st.datetimes().map(lambda dt: dt.isoformat()), st.text())
...
def test_launch_data_list_from_api(api_list):
    ...
    for api_data, launch_data_list_data in zip(api_list, ldl):
        ...
        assert datetime.datetime.fromisoformat(api_data["last_updated"]) == launch_data_list_data.last_updated
...
@given(
    ...
    st.lists(st.tuples(st.text(min_size=1), st.datetimes().map(lambda dt: dt.isoformat())), max_size=0)
)
...
@given(
    ...
    st.lists(st.tuples(st.text(min_size=1), st.datetimes().map(lambda dt: dt.isoformat())))
)
def test_launch_data_list_compare_sth_to_diff(api_list, db_list):
    db_list = [element for element in db_list if element[0] not in [api_element["id"] for api_element in api_list]]
    ldl_api = LaunchDataList.from_api(api_list)
    ldl_db = LaunchDataList.from_db(db_list)
    assert list(ldl_api.compare(ldl_db)) == ldl_api
